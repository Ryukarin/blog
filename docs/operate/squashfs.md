...
1. 首先下载 [EmuELEC 4.2 镜像包](https://github.com/EmuELEC/EmuELEC/releases/download/v4.2/EmuELEC-Amlogic.aarch64-4.2-Generic.img.gz)

2. 解压镜像包。右击下载好的镜像包，选择快捷菜单 **<u>提取到此处</u>**；或者用下面命令解压缩

   ```shell
   kei@kei-KVM:~$ gunzip EmuELEC-Amlogic.aarch64-4.2-Generic.img.gz
   ```

3. 右击解压出来的镜像 `EmuELEC-Amlogic.aarch64-4.2-Generic.img` 文件，选择快捷菜单的 <u>**用 磁盘映像挂载器 打开**</u> 进行挂载，会在桌面显示挂载的盘（STORAGE 和 EMUELEC）
...
```shell
# 进入 squashfs-root 目录并查看一下
root@kei-KVM:~# cd squashfs-root/
root@kei-KVM:~/squashfs-root# ls -l
总用量 44
