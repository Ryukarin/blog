...
<span class="token punctuation">..</span>.省略部分输出<span class="token punctuation">..</span>.
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br></div></div><p>这个文件的每行代表一个用户，同样使用“:”作为分隔符，划分为 9 个字段。我们以 root 行为例，这 9 个字段的作用如下。</p>
<ol>
<li><strong>用户名称</strong></li>
</ol>
...
<div class="custom-container tip"><p class="custom-container-title">注意</p>
<p>所有伪用户的密码都是“!!”或“*”，代表这些用户都没有密码，是不能登录的。当然，新创建的用户如果不设定密码，那么它的密码项也是“!!”，代表这个用户没有密码，不能登录。</p>
</div>
<ol start="3">
<li><strong>密码最后一次修改日期</strong></li>
</ol>
<p>第三个字段是密码的修改日期，可是这里怎么是 18768 啊？代表什么意思呢？其实 Linux 更加习惯使用时间戳代表时间，也就是说，以 <u>1970 年 1 月 1 日</u> 作为标准时间，每过去一天，时间戳加 1，那么 366 代表的就是 1971 年1 月 1 日。我们这里的时间戳是 18768，也就是说，是在 1970 年1 月 1 日之后的第 15775 天修改的 root 用户密码。好隐晦的表示啊！那么，到底 18768 的时间戳代表的是哪一天呢？我们可以使用如下命令进行换算：</p>
...
<p>第四个字段是密码的两次修改间隔时间。这个字段要<u>和第三个字段相比</u>，也就是说密码被修改后多久不能再修改密码。如果是 0，则密码可以随时修改。如果是 10，则代表密码修改后10 天之内不能再次修改这个密码。</p>
<ol start="5">
<li><strong>密码的有效期</strong></li>
</ol>
<p>第五个字段是密码的有效期。这个字段也要</u>和第三个字段相比</u>，也就是说密码被修改后可以生效多少天。默认值是 99999，也就是 273 年，大家可以认为永久生效。如果改为 90，那么密码被修改 90 天之后就必须再次修改，否则该用户就不能登录了。我们在管理服务器的时候可以通过这个字段强制用户定期修改密码。</p>
<ol start="6">
<li><strong>密码修改到期前的警告天数</strong></li>
</ol>
<p>第六个字段是密码修改到期前的警告天数。这个字段要<u>和第五个字段相比</u>，就是密码到期前需提前几天修改。默认值是 7，也就是说从密码到期前的7 天开始，每次登录系统都会警告该用户修改密码。</p>
<ol start="7">
<li><strong>密码过期后的宽限天数</strong></li>
</ol>
<p>第七个字段是密码过期后的宽限天数。这个字段要<u>和第五个字段相比</u>，也就是密码过期后，用户如果还是没有修改密码，那么在宽限天数内用户还是可以登录系统的；如果过了宽限天数，那么用户就无法再使用该器码登录了。天数如果是 10，则代表密码过期 10 天后失效；如果是 0，则代表密码过期后立叫失效；如果是 -1，则代表密码永远不会失效。</p>
<ol start="8">
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br></div></div><p>这个文件同样使用“:”作为分隔符，把文件划分为 4 个字段，每个字段的含义如下。</p>
<ol>
<li><strong>组名</strong></li>
</ol>
<p>第一个字段是这个用户的组名。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p>在 Linux 中，家目录用“~”表示，当前命令的提示符是“[root@localhost ~]#”，表示当前所在目录就是家目录。而我当前是超级用户，所以我的家目录就是 <code>/root/</code>。</p>
<ol start="2">
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p>当然，这个用户还没有设置密码，所以密码字段是“!!”，代表这个用户没有合理密码，不能正常登录。同时会按照默认值设定时间字段。</p>
